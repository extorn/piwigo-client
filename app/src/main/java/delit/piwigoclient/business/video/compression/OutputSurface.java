...
class OutputSurface implements SurfaceTexture.OnFrameAvailableListener {
     ...
     */
    public OutputSurface(int width, int height, Handler h) {
        if (width <= 0 || height <= 0) {
     ...
     */
    public OutputSurface(Handler h) {
        setup(h);
    }
    ...
    private void setup(Handler h) {
        ...
        if (h != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mSurfaceTexture.setOnFrameAvailableListener(this, h);
        } else {
     ...
     * data is available.
     */
    public void awaitNewImage(long timeoutMillis) {
        ...
        synchronized (mFrameSyncObject) {
            while (!mFrameAvailable) {
                try {
                    ...
                    mFrameSyncObject.wait(timeoutMillis);
                    if (!mFrameAvailable) {
