...
public class PreferenceMigrator256Paid extends PreferenceMigrator {
    ...
    protected void upgradePreferences(Context context, SharedPreferences prefs, SharedPreferences.Editor editor) {
        ...
        Logging.log(Log.INFO, TAG, "found auto upload jobs : " + autoUploadJobs.size());
        for (AutoUploadJobConfig cfg : autoUploadJobs) {
            try {
                Set<String> items = cfg.getFileExtsToUpload(context);
                Logging.log(Log.INFO, TAG, "auto upload job set to upload exts : " + CollectionUtils.toCsvList(items));
            } catch (ClassCastException e) {
                Logging.log(Log.INFO, TAG, "auto upload job preference needs upgrade");
                ...
                Logging.log(Log.INFO, TAG, "auto upload job preference extracted and ready for migration");
                ...
                Logging.log(Log.INFO, TAG, "auto upload job preference upgraded from string (" + fileExtsCsvList + ") to string set (" + CollectionUtils.toCsvList(cleanedValues) + ")");
            }
        }
    }
}
